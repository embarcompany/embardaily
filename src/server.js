...
function supabaseContactDto(row) { return { row: row.id, client: row.client, pet: row.pet, phone: row.phone, company: row.company, destination: row.destination, date: row.date, notes: row.summary, folderUrl: row.folder_url, contacted: Boolean(row.touch_1_at), touch1At: row.touch_1_at, replyAt: row.reply_at, reminderAttempts: Number(row.reminder_count || 0), status: row.status || "nao_iniciado", nextActionAt: row.next_action_at, nextActionLabel: row.next_action_label }; }
function contactDto(row) { return { row: row._rowNumber, client: row.Cliente, pet: row.Pet, phone: row.Contato, company: row.Empresa, destination: row.Destino, date: row["Data do embarque"], notes: row.Observações, testimonial: row.texto_depoimento, folderUrl: row["Pasta no Drive"], contacted: contacted(row), touch1At: row.data_toque1, touch2At: row.data_toque2, replyAt: row.data_resposta_toque1, reminderAttempts: Number(row.tentativas_lembrete || 0), status: row.status_embardaily || "não_iniciado" }; }
...
http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/shipments" && supabase.enabled()) { try { const input = JSON.parse(await jsonBody(req)); if (!input.client || !input.phone || !input.date || !input.pets?.length) throw new Error("Preencha tutor, telefone, data e ao menos um pet."); const shipment = await supabase.createShipment(input); res.setHeader("content-type", "application/json"); return res.end(JSON.stringify({ row: shipment.id })); } catch (error) { res.writeHead(422); return res.end(error.message); } }
  if (req.method === "GET" && url.pathname === "/api/contacts") { try { res.setHeader("content-type", "application/json"); if (supabase.enabled()) return res.end(JSON.stringify((await supabase.contacts()).map(supabaseContactDto))); if (!config.sheetId) return res.end(JSON.stringify(demoContacts)); return res.end(JSON.stringify((await sheets.rows()).map(contactDto))); } catch (error) { res.writeHead(500); return res.end(JSON.stringify({ error: error.message })); } }
  ...
  if (req.method === "PATCH" && edit) { try { const { status } = JSON.parse(await jsonBody(req)); if (supabase.enabled()) await supabase.updateStatus(edit[1], status); else await sheets.update(edit[1], { status_embardaily: status }); res.writeHead(204); return res.end(); } catch (error) { res.writeHead(500); return res.end(error.message); } }
