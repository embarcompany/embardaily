...
http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/contacts") { try { res.setHeader("content-type", "application/json"); return res.end(JSON.stringify((await sheets.rows()).map(contactDto))); } catch (error) { res.writeHead(500); return res.end(JSON.stringify({ error: error.message })); } }
  if (req.method === "POST" && url.pathname === "/api/shipments") { try { await sheets.ensureColumns(); const input = JSON.parse(await jsonBody(req)); if (!input.client || !input.phone || !input.date || !input.pets?.length) throw new Error("Preencha tutor, telefone, data e ao menos um pet."); const pets = input.pets.map((pet) => pet.name).join(", "); const breeds = input.pets.map((pet) => pet.breed).filter(Boolean).join("; "); const row = await sheets.append({ Cliente: input.client, Contato: input.phone, Pet: pets, "Espécie / Raça": breeds, Origem: input.origin || "", Destino: input.destination || "", Modalidade: input.modality || "", "Data do embarque": input.date, Empresa: input.company || "Embarpet", "Postado?": "Não", "Contato Realizado?": false, Observações: [input.notes, input.reference ? `Processo: ${input.reference}` : ""].filter(Boolean).join(" | "), status_embardaily: "não_iniciado" }); res.setHeader("content-type", "application/json"); return res.end(JSON.stringify(contactDto(row))); } catch (error) { res.writeHead(422); return res.end(error.message); } }
